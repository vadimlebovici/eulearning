...
def build_vectorized_st_from_adjacency_matrix(A, filtrations):
	...
	Output:
		vec_st 		: ndarray of size (num_vertices+num_edges, 1+len(filtrations)) containing for each simplex splx: [(-1)*dim(splx), filt_1(splx), ..., filt_n(splx)] where filt_j is the upper star filtration induced by the function on the vertices filtrations[j].
	'''
...
# Code to extract the function defined on the nodes of DHFR and save in in a .txt file. 
# This function does not need to be run again. It is placed here for completeness, in order to be adapted on other graph dataset. 
